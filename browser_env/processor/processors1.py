'''
以携程为例，修改了一版，针对没解析出来的li元素做了优化，并且把解析出来的元素做了cut
但cut有点过度，把有一些本来不会被挡住的元素也cut掉了
'''
...
class ImageObservationProcessor(ObservationProcessor):
    ...
    def get_page_bboxes(self, page: Page) -> list[list[float]]:
        ...
        (() => {
            ...
            function collectElements(root, elements = []) {
                ...
                const walker = document.createTreeWalker(
                    ...
                    NodeFilter.SHOW_ELEMENT,
                    {
                        acceptNode: (node) => {
                            // 稍微过滤一下，避免遍历太多无用节点
                            if (['SCRIPT', 'STYLE', 'NOSCRIPT', 'SVG', 'PATH'].includes(node.tagName)) return NodeFilter.FILTER_REJECT;
            ...
            // 执行遍历, 去重与嵌套过滤
            let allElements = collectElements(document.body);

            // 4. 去重逻辑 (修复版：内容优先 Content-First)
            function filterRedundantElements(elements) {
                const keepSet = new Set(elements);
                const elementData = elements.map(el => ({
                    el, 
                    rect: el.getBoundingClientRect(),
                    // 定义"原生强交互控件"：只有它们才有资格吞噬内部元素
                    // 注意：TD, LI, DIV 不在这里面！
                    isNative: ['BUTTON', 'INPUT', 'SELECT', 'TEXTAREA', 'A', 'LI'].includes(el.tagName)
                }));

                for (let i = 0; i < elementData.length; i++) {
                    const parent = elementData[i];
                    if (!keepSet.has(parent.el)) continue;

                    for (let j = 0; j < elementData.length; j++) {
                        if (i === j) continue;
                        const child = elementData[j];
                        if (!keepSet.has(child.el)) continue;

                        // 检查包含关系
                        const isContained = (
                            child.rect.top >= parent.rect.top &&
                            child.rect.left >= parent.rect.left &&
                            child.rect.bottom <= parent.rect.bottom &&
                            child.rect.right <= parent.rect.right
                        );

                        if (isContained) {
                            // 规则 1: 原生控件霸权 (Native Supremacy)
                            // 如果父级是 <button> 或 <a>，那么内部的 <span> 确实是多余的，删掉子级
                            if (parent.isNative) {
                                keepSet.delete(child.el);
                                continue;
                            }

                            // 规则 2: 视觉重叠合并 (Visual Overlap)
                            // 如果父级是普通容器 (DIV, LI, TD...)
                            // 只有当子级几乎填满了父级 (>80% 面积) 时，才认为它们是同一个东西，删掉子级
                            // 这样可以保留 TD 内部小小的 Text Span，但会合并那个包裹 Text 的 Wrapper Div
                            const parentArea = parent.rect.width * parent.rect.height;
                            const childArea = child.rect.width * child.rect.height;
                            
                            // 防止除以0
                            if (parentArea > 0 && (childArea / parentArea > 0.8)) {
                                // 额外保护：如果子级是 Input/Button，千万别删，删父级
                                if (['BUTTON', 'INPUT', 'SELECT', 'TEXTAREA'].includes(child.el.tagName)) {
                                     keepSet.delete(parent.el);
                                } else {
                                     keepSet.delete(child.el);
                                }
                            }
                ...
                return Array.from(keepSet);
            }

            allElements = filterRedundantElements(allElements);
            ...
            function filterOccludedElements(elements) {
                return elements.filter(element => {
                    ...
                    // 1. 如果视口外或找不到顶层，保守起见保留
                    if (!topElement) return true; 

                    // 2. 基础检查：如果是自己，或者是自己的子孙 -> 保留
                    if (element === topElement || element.contains(topElement)) return true;

                    // 3. 祖先检查：如果顶层是自己的祖先 (点击穿透，或者 Wrapper) -> 保留
                    if (topElement.contains(element)) return true;

                    // 4. Label 特例 -> 保留
                    if (element.tagName === 'LABEL' && element.control === topElement) return true;

                    // 5. 【核心修复】同层级/亲属宽容 (Sibling Tolerance)
                    // 如果遮挡物 (topElement) 和当前元素 (element) 在同一个父容器内，
                    // 我们认为这是 UI 组件内部的装饰性遮挡（如 active 背景、icon 盖住文字），允许通过。
                    const parent = element.parentElement;
                    if (parent) {
                        // 检查 1: 它们是直接兄弟 (Parent 包含 Top)
                        if (parent.contains(topElement)) return true;
                        
                        // 检查 2: 稍微放宽一点，允许它们是堂兄弟 (Grandparent 包含 Top)
                        // 适用于复杂的组件结构 (如 TD > DIV > SPAN vs TD > DIV > BG)
                        const grandparent = parent.parentElement;
                        if (grandparent && (grandparent.tagName === 'TD' || grandparent.tagName === 'LI' || grandparent.tagName === 'DIV')) {
                             if (grandparent.contains(topElement)) return true;
                        }
                    }

                    // 6. 如果到了这一步，说明 topElement 是一个完全不相关的外人（比如 Modal 遮罩）
                    // 此时确实被遮挡了，剔除。
                    return false;
                });
            }

            allElements = filterOccludedElements(allElements);
