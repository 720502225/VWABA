...
def main():
    with sync_playwright() as p:
        ...
        text_processor = TextObervationProcessor(
            observation_type="accessibility_tree", 
            current_viewport_only=True,
            viewport_size=viewport_size
        )
        # Create a SOM processor
        ...
        while True:
            ...
            try:
                ...
                text_obs_content = text_processor.process(page)
                nodes_info = text_processor.obs_nodes_info
                print(f"   解析完成，捕获到 {len(nodes_info)} 个交互节点。")
                #print(text_obs_content)
                screenshot_bytes = page.screenshot()
                screenshot_img = Image.open(io.BytesIO(screenshot_bytes))
                som_image_array, id2center = image_processor.draw_som_from_nodes(
                    screenshot_img,
                    nodes_info,
                    viewport_size
                )
